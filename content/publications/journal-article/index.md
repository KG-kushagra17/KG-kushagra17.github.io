...
Before Transformers, sequence models like RNNs and LSTMs processed words one by one.
...
Attention solved this problem by allowing every word to directly look at every other word. Instead of remembering everything through a long chain, the model can simply ask: “Which words are relevant to me right now?”
...
Compare it with labels on shelves
...
Every token creates a query vector. If the token is “sat,” its query might implicitly ask: “Who is doing the sitting?”
...
A larger dot product means: These vectors align strongly.

So if the query from “sat” strongly matches the key from “cat,” the model decides: “cat is important for understanding sat.”

Value (V)
The Value contains the actual information passed forward. We can think of it this way:
...
After determining importance scores, the model combines Values using weighted averaging. Important words contribute more; unimportant words contribute less.
...
Query asks: “What information am I looking for?” * Query and Key dot product measures relevance between words.

Values are weighted by softmax scores to create the final context-aware representation.

A Simple Attention Diagram
Here is a simplified diagram of attention flow:
...
Now comes the next big idea. Instead of doing attention once, Transformers do it multiple times in parallel. These are called attention heads.
...
Attention removed the sequential bottleneck of RNNs. Transformers gained major advantages:
